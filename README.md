...
为 [Claude Code](https://docs.anthropic.com/en/docs/claude-code) 打造的持久化 AI 记忆系统。自动从每次会话中提取知识，基于使用频率智能遗忘与再激活。

## 功能特性

- **自动提取**: 会话结束时调用 Claude Sonnet 从对话记录中提取关键知识、技能、模式和解决方案
- **自动加载**: 新会话开始时将所有活跃记忆注入上下文，Claude 可直接引用历史知识
- **智能合并**: 新提取的内容与现有记忆去重，相似条目自动合并而非重复存储
...
会话结束 ────→ 读取对话记录 (transcript)
                    │
              调用 Sonnet API 提取结构化知识
                    │
              与 global_memory 合并去重
                    │
...
| 类别 | 说明 |
|------|------|
| `knowledge` | 事实、配置、系统行为 |
| `skill` | 方法、技巧、工作流程 |
| `pattern` | 代码或运维中的重复模式 |
| `solution` | 问题 → 解决方案 |
| `reference` | 外部资源、文档、URL |
